		...
		<b-container class="mt-md-5 mt-4 mb-md-5 mb-4">
			<div class="faq_cont_body">
				...
				<div>
					...
					<b-card no-body class="faq_cont fixed_head mb-0" v-for="list in faqFixedListData" :v-Key="faqFixedListData" v-if="fixedNotice == true">
						<b-card-header header-tag="header" class="p-0" role="tab">
							<div class="faqcontbox" block v-b-toggle="'accordion-' + list.idx">
								<div class="num" :idx="list.idx">
									<div v-if="$mq == 'desktop'">공지</div>
									<div v-else>공지 / {{dayLength(list.createDate)}}</div>
								</div>
			...
			<div class="search_cont">
				<b-form-input v-model="searchText"  @keyup.enter="searchBtn" placeholder=""></b-form-input>
	...
	export default{
		data(){
			return {
				...
				fixedNotice: false,
				seviceTarget: '',
				userBoardFaqPage: '/users/board/notice/notFixed/?page=',
				userBoardFixedFaqPage: '/users/board/notice/fixed/?page=0',
				userBoardFaqSearch: '/users/board/notice/search/',
				currentPage: 1,
				listlp: 10,
			}
		},
	    ...
	    methods: {
			faqList() {
				axios
	            .get(process.env.VUE_APP_BASE_URL + this.userBoardFaqPage + this.$route.query.page+'&size='+this.listlp+'', )
	            .then( res => {
			...
			searchBtn() {
				if(this.searchText == ''){
					...
					axios
		            .get(process.env.VUE_APP_BASE_URL + this.userBoardFaqSearch + this.searchText+'/?page=0&size='+this.listlp+'&sort=idx,desc', { withCredentials: true })
		            .then( res => {
		...
		watch: {
			$route (data){
				...
				if(this.seviceTarget == 'pagination'){
					this.searchText = ''
					linkAddress = process.env.VUE_APP_BASE_URL+this.userBoardFaqPage+ this.$route.query.page+'&size='+this.listlp+''
				} else {
					linkAddress = process.env.VUE_APP_BASE_URL+this.userBoardFaqSearch+this.searchText+'/?page='+ this.$route.query.page+'&size='+this.listlp+'&sort=idx,desc'
				}
				axios
...
.faq_cont_head .num{float:left;width:65px;text-align:center;}
.faq_cont_head .title{float:left;width:calc(100% - 185px);}
.faq_cont_head .day{float:left;width:120px;text-align:center;}
.card-header .btn > div{padding:10px;box-sizing:border-box;}
.faq_cont.fixed_head .faqcontbox{background-color:#4d7ee1;}
.faq_cont.fixed_head .faqcontbox >>> div{color:#fff;}
